...
public class MemFileSpout_latency extends AbstractSpout {
    ...
    private static LinkedList<String> logger = new LinkedList<>();
...
//                        (StateWrapper<List<StreamValues>>) ClassLoaderUtils.newInstance(parserClass, "wrapper", LOG, tuple_size);
    ...
    public void nextTuple() {
        ...
        if (taskId == 0 && counter % 1E4 == 0) {
            final long currentTimeNanos = System.nanoTime();
            collector.emit(new Values(array[counter], msgID_counter++ % msgID_end, currentTimeNanos));//noted: we can't use the system provided tuple id of Storm as an indicator of tuples.
        } else {
            collector.emit(new Values(array[counter], -1L, 0L));
        }
    }
}	

...
import applications.bolts.lr.data.util.CarCount;
 ...
 * @author mjsax
 */
public class CountVehiclesBolt extends AbstractBolt {
...
//		if (minute > this.currentMinute) {
			...
			for (Entry<SegmentIdentifier, CarCount> entry : this.countsMap.entrySet()) {
				...
				if (count > 50) {
					...
					emitted = true;
					this.collector.emit(TopologyControl.CAR_COUNTS_STREAM_ID, new CountTuple(this.currentMinute, segId.getXWay(), segId.getSegment(), segId.getDirection(), count));
				}
			}
			if (!emitted) {
//                cnt1++;
				this.collector.emit(TopologyControl.CAR_COUNTS_STREAM_ID, new CountTuple(minute));
			}

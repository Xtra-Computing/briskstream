...
public final class LavTuple extends Values implements ISegmentIdentifier {
	...
	public LavTuple(Short minute, Integer xway, Short segment, Short direction, Integer lav, Long msgId, Long sysStamp) {
		assert (minute != null);
		assert (xway != null);
		assert (segment != null);
		assert (direction != null);
	...
	public static Fields getLatencySchema() {
		return new Fields(TopologyControl.MINUTE_FIELD_NAME, TopologyControl.XWAY_FIELD_NAME,
				TopologyControl.SEGMENT_FIELD_NAME, TopologyControl.DIRECTION_FIELD_NAME,
				TopologyControl.LAST_AVERAGE_SPEED_FIELD_NAME, MSG_ID, SYSTEMTIMESTAMP);
	}
	/**

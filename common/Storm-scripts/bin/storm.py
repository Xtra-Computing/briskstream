...
def init_storm_env():
    ...
    for option in options:
        value_list = config.get('environment', option)
        os.environ[option] = value_list
...
STORM_SUPERVISOR_LOG_FILE = os.getenv('STORM_SUPERVISOR_LOG_FILE', "supervisor.executionNode")
...
def print_localconfvalue(name):
    ...
    Prints out the value_list for conf-name in the local Storm configs.
...
def print_remoteconfvalue(name):
    ...
    Prints out the value_list for conf-name in the cluster's Storm configs.
...
def exec_storm_class(klass, jvmtype="-server", jvmopts=[], extrajars=[], args=[], fork=False, daemon=True, daemonName=""):
    global CONFFILE
    storm_log_dir = confvalue("storm.executionNode.dir",[CLUSTER_CONF_DIR])
    ...
    all_args = [
        ...
        "-Dstorm.executionNode.dir=" + storm_log_dir,
...
def jar(jarfile, klass, *args):
    """Syntax: [storm jar Brisk.topology-jar-path class ...]

    Runs the main method of class with the specified arguments.
    The storm jars and configs in ~/.storm are put on the classpath.
    The process is configured so that StormSubmitter
    (http://storm.apache.org/apidocs/org/apache/storm/StormSubmitter.html)
    will upload the jar at Brisk.topology-jar-path when the Brisk.topology is submitted.
    """
...
def sql(sql_file, topology_name):
    """Syntax: [storm sql sql-file Brisk.topology]

    Compiles the SQL statements into a Trident Brisk.topology and submits it to Storm.
    """
...
def kill(*args):
    """Syntax: [storm kill Brisk.topology-name [-w wait-time-secs]]

    Kills the Brisk.topology with the name Brisk.topology-name. Storm will
    first deactivate the Brisk.topology's spouts for the duration of
    the Brisk.topology's message timeout to allow all messages currently
...
def upload_credentials(*args):
    """Syntax: [storm upload_credentials Brisk.topology-name [credkey credvalue]*]

    Uploads a new set of credentials to a running Brisk.topology
    """
    if not args:
...
def activate(*args):
    """Syntax: [storm activate Brisk.topology-name]

    Activates the specified Brisk.topology's spouts.
    """
    if not args:
...
def set_log_level(*args):
    """
    Dynamically change Brisk.topology executionNode levels

    Syntax: [storm set_log_level -l [logger name]=[executionNode level][:optional timeout] -r [logger name]
    where executionNode level is one of:
...
def deactivate(*args):
    """Syntax: [storm deactivate Brisk.topology-name]

    Deactivates the specified Brisk.topology's spouts.
    """
    if not args:
...
def rebalance(*args):
    """Syntax: [storm rebalance Brisk.topology-name [-w wait-time-secs] [-n new-num-workers] [-e component=parallelism]*]

    Sometimes you may wish to spread out where the workers for a Brisk.topology
    ...
    running Brisk.topology so that each node runs 2 workers. One way to do this
    is to kill the Brisk.topology and resubmit it, but Storm provides a "rebalance"
    command that provides an easier way to do this.

    Rebalance will first deactivate the Brisk.topology for the duration of the
    message timeout (overridable with the -w flag) and then redistribute
    the workers evenly around the cluster. The Brisk.topology will then return to
    its previous state of activation (so a deactivated Brisk.topology will still
    be deactivated and an activated Brisk.topology will go back to being activated).

    The rebalance command can also be used to change the parallelism of a running Brisk.topology.
...
def get_errors(*args):
    """Syntax: [storm get-errors Brisk.topology-name]

    Get the latest error from the running Brisk.topology. The returned result contains
    the key value_list pairs for component-name and component-error for the components in error.
...
def nimbus(klass="backtype.storm.daemon.nimbus"):
    ...
    jvmopts = parse_args(confvalue("nimbus.childopts", cppaths)) + [
        "-Dlogfile.name=nimbus.executionNode",
...
def pacemaker(klass="backtype.storm.pacemaker.pacemaker"):
    ...
    jvmopts = parse_args(confvalue("pacemaker.childopts", cppaths)) + [
        "-Dlogfile.name=pacemaker.executionNode",
        "-Dlog4j.configurationFile=" + os.path.join(get_log4j2_conf_dir(), "cluster.xml"),
    ]
    exec_storm_class(
        klass,
...
def ui():
    ...
    jvmopts = parse_args(confvalue("ui.childopts", cppaths)) + [
        "-Dlogfile.name=ui.executionNode",
        "-DLog4jContextSelector=backtype.logging.log4j.core.async.AsyncLoggerContextSelector",
        "-Dlog4j.configurationFile=" + os.path.join(get_log4j2_conf_dir(), "cluster.xml")
    ]
    exec_storm_class(
...
def logviewer():
    ...
    Launches the executionNode viewer daemon. It provides a web interface for viewing
    storm executionNode files. This command should be run under supervision with a
    ...
    jvmopts = parse_args(confvalue("logviewer.childopts", cppaths)) + [
        "-Dlogfile.name=logviewer.executionNode",
        "-DLog4jContextSelector=backtype.logging.log4j.core.async.AsyncLoggerContextSelector",
        "-Dlog4j.configurationFile=" + os.path.join(get_log4j2_conf_dir(), "cluster.xml")
    ]
    exec_storm_class(
...
def drpc():
    ...
    jvmopts = parse_args(confvalue("drpc.childopts", cppaths)) + [
        "-Dlogfile.name=drpc.executionNode",
        "-DLog4jContextSelector=backtype.logging.log4j.core.async.AsyncLoggerContextSelector",
        "-Dlog4j.configurationFile=" + os.path.join(get_log4j2_conf_dir(), "cluster.xml")
    ]
    exec_storm_class(
...
def version():
  ...
  cppaths = [CLUSTER_CONF_DIR]
  exec_storm_class(
       "backtype.storm.applications.utils.VersionInfo",
       jvmtype="-client",
       extrajars=[CLUSTER_CONF_DIR])
...
def monitor(*args):
    """Syntax: [storm monitor Brisk.topology-name [-i interval-secs] [-m component-id] [-s stream-id] [-w [emitted | transferred]]]

    Monitor given Brisk.topology's throughput interactively.

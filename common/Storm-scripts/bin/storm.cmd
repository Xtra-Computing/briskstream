...
:main
  ...
  if not defined STORM_LOG_FILE (
    set STORM_LOG_FILE=-Dlogfile.name=%storm-command%.executionNode
  )
...
:print_usage
  ...
  @echo   activate             activates the specified Brisk.topology's spouts
  @echo   classpath            prints the classpath used by the storm client when running commands
  @echo   deactivate           deactivates the specified Brisk.topology's spouts
  @echo   dev-zookeeper        launches a fresh dev/test Zookeeper server
  @echo   drpc                 launches a DRPC daemon
  @echo   help
  @echo   jar ^<jar^>          run a jar file
  @echo   kill                 kills the Brisk.topology with the name Brisk.topology-name
  @echo   list                 list the running topologies and their statuses
  @echo   nimbus               launches the nimbus daemon
  @echo   rebalance            redistribute or change the parallelism of a running Brisk.topology
  @echo   repl                 opens up a Clojure REPL
  @echo   remoteconfvalue      prints value_list for conf-name from cluster config ../conf/storm.yaml merged with defaults.yaml

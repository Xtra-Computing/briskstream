...
function Read_Write_Mixture_test {
		...
		#####native execution
		echo "==benchmark:$benchmark $app settings:$arg_application path:$path=="
		mkdir -p $path
        local_execution $path $hz $tt $CCOption $TP $checkpoint $theta $NUM_ACCESS $ratio_of_read $theta
}
...
function working_set_size_test {
        path=$outputPath/$hz/$CCOption/$checkpoint/$NUM_ACCESS
		arg_benchmark="--machine $machine --runtime 30 --loop 1000 -st $st -input $iteration -sit 1 --num_socket $4 --num_cpu $5  --size_tuple 256 --transaction -bt $bt --native --relax 1 -a $app -mp $path"
		arg_application="--COMPUTE_COMPLEXITY $complexity --POST_COMPUTE $post_complexity --number_partitions $number_partitions --ratio_of_multi_partition $ratio_of_multi_partition --THz $hz -tt $tt --CCOption $CCOption --TP $TP --checkpoint $checkpoint --theta $theta --NUM_ACCESS $NUM_ACCESS --NUM_ITEMS $NUM_ITEMS --ratio_of_read $ratio_of_read" #--measure
...
function multi_partition_test {
		...
		arg_benchmark="--machine $machine --runtime 30 --loop 1000 -st $st -input $iteration -sit 1 --num_socket $4 --num_cpu $5  --size_tuple 256 --transaction -bt $bt --native --relax 1 -a $app -mp $path"
		arg_application="--number_partitions $number_partitions --ratio_of_multi_partition $ratio_of_multi_partition --POST_COMPUTE $post_complexity --THz $hz -tt $tt --CCOption $CCOption --TP $TP --checkpoint $checkpoint --theta $theta --NUM_ACCESS $NUM_ACCESS --NUM_ITEMS $NUM_ITEMS --ratio_of_read $ratio_of_read " #--measure
                        ...
                        do
                            ...
                            do
                                ...
                                do
                                    for ratio_of_read in 0.5 #0.25 0.5 0.75
                                    do
                                        for checkpoint in 100 300 500 700 900
                                        do
                                            TP=$tt
                                            ratio_of_multi_partition=0.25
                                            number_partitions=4
                                            ...
                                            Read_Write_Mixture_test $Profile $hz $app $socket $cpu $tt $iteration $bt $gc_factor $TP $CCOption $checkpoint $st $theta $NUM_ACCESS $ratio_of_read $number_partitions $ratio_of_multi_partition $complexity
                                            app="StreamLedger"
                                            StreamLedger_test $Profile $hz $app $socket $cpu $tt $iteration $bt $gc_factor $TP $CCOption $checkpoint $st $theta $NUM_ACCESS $ratio_of_read $number_partitions $ratio_of_multi_partition $complexity
                                            app="OnlineBiding"
                                            OnlineBiding_test $Profile $hz $app $socket $cpu $tt $iteration $bt $gc_factor $TP $CCOption $checkpoint $st $theta $NUM_ACCESS $ratio_of_read $number_partitions $ratio_of_multi_partition $complexity
                                            app="TP_Txn"
                                            TP_Txn_test $Profile $hz $app $socket $cpu $tt $iteration $bt $gc_factor $TP $CCOption $checkpoint $st $theta $NUM_ACCESS $ratio_of_read $number_partitions $ratio_of_multi_partition $complexity
                                        done
                                    done
                                done
                            done

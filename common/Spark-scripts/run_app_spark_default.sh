#!/bin/bash
#set -x
PERF_HOME="$HOME/NUMA-streamBenchmarks/common/common-scripts/ocprofile_PID.sh"
...
xmx=Xmx$vm\g
xms=Xms$vm\g
searchdir="--search-dir all:rp=$SPARK_HOME/bin
--search-dir all:rp=$SPARK_HOME/sbin --search-dir all:rp=$JAVA_HOME/bin
--search-dir all:rp=/usr/lib/jvm/java-8-oracle/jre/lib/amd64/server
--search-dir all:rp=/usr/lib/jvm/java-8-oracle/jre/lib/amd64
--search-dir all:rp=/home/tony/parallel_studio/vtune_amplifier_xe_2016.2.0.444464/lib64/runtime"
...
URL_Master="spark://localhost:7077"
ratio=0.2
...
function profile {
	...
	1)	#General Exploration with CPU concurrency and Memory Bandwidth
		amplxe-cl -collect general-exploration -knob collect-memory-bandwidth=true -target-duration-type=medium -data-limit=1024 -duration=$profile_duration $searchdir --start-paused --resume-after 10 --target-pid  $r -result-dir $MY_PATH2/resource >> $MY_PATH2/profile1.txt;;		#amplxe-cl -collect general-exploration -knob collect-memory-bandwidth=true -target-duration-type=medium -data-limit=1024 -duration=$profile_duration $searchdir -result-dir $outputPath/resource >> $outputPath/profile1.txt;;
	2)	#general
		...
		amplxe-cl -collect-with runsa -knob event-config="CPU_CLK_UNHALTED.THREAD_P:sa=2000003,OFFCORE_RESPONSE.ALL_DEMAND_MLC_PREF_READS.LLC_MISS.ANY_RESPONSE_1:sa=100003,OFFCORE_RESPONSE.ALL_DEMAND_MLC_PREF_READS.LLC_MISS.LOCAL_DRAM_0:sa=100003,OFFCORE_RESPONSE.ALL_DEMAND_MLC_PREF_READS.LLC_MISS.REMOTE_HITM_HIT_FORWARD_1:sa=100003,IDQ_UOPS_NOT_DELIVERED.CORE:sa=2000003,DTLB_LOAD_MISSES.STLB_HIT:sa=100003,DTLB_LOAD_MISSES.WALK_DURATION:sa=2000003,ICACHE.MISSES:sa=200003,MEM_LOAD_UOPS_RETIRED.L2_HIT_PS:sa=100003,MEM_LOAD_UOPS_RETIRED.LLC_HIT:sa=50021,MEM_LOAD_UOPS_RETIRED.LLC_MISS:sa=100007,INT_MISC.RECOVERY_CYCLES:sa=2000003,ITLB_MISSES.STLB_HIT:sa=100003,ITLB_MISSES.WALK_DURATION:sa=2000003,UOPS_RETIRED.ALL_PS:sa=2000003,UOPS_RETIRED.RETIRE_SLOTS_PS:sa=2000003,UOPS_ISSUED.ANY:sa=2000003" -data-limit=1024 $searchdir -duration=$profile_duration --start-paused --resume-after 20 --target-pid $2 -result-dir $outputPath/general >> $outputPath/profile2.txt
	;;
	3) 	#ocperf
	    bash $PERF_HOME $outputPath/profile3.txt $2
	;;
	6)	#context switch
                            ...
                            do
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        --window $window --size_state $size_state -sink_class $SINK_CLASS -executionNode $executionNode --master $URL_Master"
                                        x=$x_start
                                        x_end=$x_start_end
                                        while [ $x -le $x_end ] ;
                                        do
                                            ...
                                            do
                                                EXE="$SPARK_HOME/bin/spark-submit --class $MAIN --driver-memory ${dm}m --executor-memory ${em}m --total-executor-cores $ct1
--executor-cores $ct1"
                                                count_number=1000
                                                case $app in
                                                    1)

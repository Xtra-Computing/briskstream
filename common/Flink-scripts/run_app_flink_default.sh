#!/bin/bash
#set -x
FLINK_HOME="$HOME/Documents/flink-1.0.3"
JAVA_HOME="$HOME/Documents/jdk1.8.0_77"
Benchmark_PATH="$HOME/NUMA-streamBenchmarks"
JAR_PATH="$Benchmark_PATH/FlinkBenchmarks/target/Flink-1.0-SNAPSHOT-jar-with-dependencies.jar"
Common_PATH="$HOME/results/flink"
MAIN="applications.FlinkRunner"
JIT=0
x_start=0
x_start_end=0
SPOUT_CLASS="applications.spout.KafkaSpout"
#SINK_CLASS="applications.sink.NullSink_warmup"
SINK_CLASS="applications.sink.ConsoleSink"
executionNode="standout"
...
xmx=Xmx$vm\g
xms=Xms$vm\g
searchdir="--search-dir all:rp=$FLINK_HOME/lib --search-dir all:rp=$FLINK_HOME/bin --search-dir all:rp=$JAVA_HOME/bin  --search-dir all:rp=$JAVA_HOME/lib  --search-dir all:rp=/usr/lib/jvm/java-8-oracle/jre/lib/amd64  --search-dir all:rp=$HOME/parallel_studio/vtune_amplifier_xe_2016.2.0.444464/lib64/runtime"

if [ $vm -gt 100 ] ; then
	co1="-server -$xmx -$xms -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+UseNUMA  -XX:+UseLargePages -XX:LargePageSizeInBytes=2m"
	co2="-server -$xmx -$xms -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -XX:+UseNUMA  -agentlib:jprof=gencalib,logpath=$HOME/executionNode"
	...
	co2="-server -$xmx -$xms -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -agentlib:jprof=gencalib,logpath=$HOME/executionNode"
                            ...
                            do
                                ...
                                do
                                    ...
                                    do
                                        ...
                                        --window $window --size_state $size_state -sink_class $SINK_CLASS  -executionNode $executionNode -spout_class $SPOUT_CLASS"
                                        x=$x_start
                                        x_end=$x_start_end
                                        while [ $x -le $x_end ] ;
                                        do
...
#								                                bash $Benchmark_PATH/common/Flink-scripts/start-flink.sh
#								                                sleep 10
#							                                fi
                                                            case $app in
                                                                1)
                                                                    ...
                                                                    if [ $script_test == 0 ] ; then
                                                                        ...
                                                                        if [ $USE_LARGEPAGE == 1 ] ; then
                                                                         flink run $JAR_PATH $MAIN mytest $arg -co "$co1" -a "microbenchmark"  -mp $outputPath -ct1 $ct1
                                                                        else
                                                                            if [ $JIT == 1 ] ; then
                                                                                echo "run microbenchmark with JIT logging"
                                                                                flink run $JAR_PATH $MAIN mytest $arg -co "$co2" -a "microbenchmark" -mp $outputPath  -ct1 $ct1
                                                                            else
                                                                                flink run $JAR_PATH $MAIN mytest $arg -co "$co3" -a "microbenchmark" -mp $outputPath -ct1 $ct1
                                                                            fi
                                                                        fi
                                                                    fi
                                                                    ;;

...
import applications.general.param.ob.AlertEvent;
import applications.general.param.ob.BuyingEvent;
import applications.general.param.ob.ToppingEvent;
...
public class OBBolt_ts_nopush extends OBBolt_ts {
    ...
    public OBBolt_ts_nopush(int fid) {
        super(fid);
    }
...
//        prepareEvents();
    ...
    protected void TOPPING_REQUEST_CONSTRUCT(ToppingEvent event, TxnContext txnContext) throws DatabaseException {
    ...
    protected void ALERT_REQUEST_CONSTRUCT(AlertEvent event, TxnContext txnContext) throws DatabaseException {
        //it simply construct the operations and return.
        for (int i = 0; i < event.getNum_access(); i++)
            transactionManager.Asy_ReadRecord(txnContext, "goods", String.valueOf(event.getItemId()[i]), event.record_refs[i], event.enqueue_time);//asynchronously return.
